...
function negamaxScores(game, maxDepth = RECURSIVE_DEPTH_LIMIT) {
  ...
  const bestScores = Array.from({ length: game.width }, (_, col) => {
    ...
    return negamax(board, col, maxDepth);
  });

  return bestScores;
}

function negamax(game, nextPlay, maxDepth, depth = 0) {  
  ...
  for(let c = 0; c < game.width; c++) {
    const board = recreateBoard(game, movesToReset);
    const testScore = negamax(board, c, maxDepth, depth + 1);
